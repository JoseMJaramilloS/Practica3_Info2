...
string text2bin(string texto){//convierte el texto a binario
    ...
    int n;
    for (unsigned long long int i=0;i < texto.size();i++) {//cadena.size: retorna el tamaño de la cadena
...
string strInverter(string original){//invierte una cadena tipo string
    string invertida;
    for (long long int i=original.size()-1;i>=0;i--) {
...
string notStr(string particion){//invierte los bits de una cadena string
    ...
    for (unsigned long long int i=0;i<particion.size();i++) {
...
string codecMethod1(string particion, string aux){//Esta funcion aplica las 3 reglas del metodo 1, basandose en el numero de 0s y 1s de la particion anterior (aux)
    ...
    for (unsigned long long int i=0;i<aux.size();i++) {//Conteo de 0s y 1s de la particion anterior
        if(aux[i]=='1') count++;
    }
    ...
    if (count == aux.size()/2 && aux.size()%2 == 0){//Caso 1 '0s'=='1s' (restriccion de impares)
        ...
        for (unsigned long long int i=0,k=0;i<particion.size();i++) {
...
{   string codificado;
    ...
    for (unsigned long long int i=0;i<particion.size()-1;i++) {//Guardo a partir de ahí en orden
...
{   string decodificado;
    ...
    for (unsigned long long int i=1;i<particion.size();i++) {//Guardo a partir del segundo bit en orden
...
void strComparate(string texto1, string texto2)//Compara dos cadenas de strings
{
    if(texto1.size()==texto2.size()){
        for (unsigned long long int i=0;i<texto1.size();i++) {
